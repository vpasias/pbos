...
# Node 1
./kvm-install-vm create -c 6 -m 32768 -d 120 -t ubuntu2004 -f host-passthrough -k /root/.ssh/id_rsa.pub -l /mnt/extra/virt/images -L /mnt/extra/virt/vms -b mgmt -T US/Eastern -M 52:54:00:8a:8b:c1 node-1

# Node 2
./kvm-install-vm create -c 6 -m 32768 -d 120 -t ubuntu2004 -f host-passthrough -k /root/.ssh/id_rsa.pub -l /mnt/extra/virt/images -L /mnt/extra/virt/vms -b mgmt -T US/Eastern -M 52:54:00:8a:8b:c2 node-2

# Node 3
./kvm-install-vm create -c 6 -m 32768 -d 120 -t ubuntu2004 -f host-passthrough -k /root/.ssh/id_rsa.pub -l /mnt/extra/virt/images -L /mnt/extra/virt/vms -b mgmt -T US/Eastern -M 52:54:00:8a:8b:c3 node-3

sleep 60

virsh list --all && brctl show && virsh net-list --all

for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i 'echo "root:gprm8350" | sudo chpasswd'; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i 'echo "ubuntu:kyax7344" | sudo chpasswd'; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo sed -i 's/PasswordAuthentication no/PasswordAuthentication yes/' /etc/ssh/sshd_config"; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo sed -i 's/#PermitRootLogin prohibit-password/PermitRootLogin yes/' /etc/ssh/sshd_config"; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo systemctl restart sshd"; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo rm -rf /root/.ssh/authorized_keys"; done
...
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo chmod -x /etc/update-motd.d/*"; done

for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i 'cat << EOF | sudo tee /etc/update-motd.d/01-custom
#!/bin/sh
echo "****************************WARNING****************************************
...
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo DEBIAN_FRONTEND=noninteractive apt-get install linux-generic-hwe-20.04 --install-recommends -y"; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo apt autoremove -y && sudo apt --fix-broken install -y"; done

for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo mkdir -p /etc/systemd/system/networking.service.d"; done
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "cat << EOF | sudo tee /etc/systemd/system/networking.service.d/reduce-timeout.conf
[Service]
TimeoutStartSec=15
EOF"; done
...
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "sudo apt update -y"; done
...
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "cat << EOF | sudo tee /etc/hosts
127.0.0.1 localhost
...
for i in {1..3}; do ssh -o "StrictHostKeyChecking=no" ubuntu@node-$i "cat << EOF | sudo tee /etc/sysctl.d/60-lxd-production.conf
