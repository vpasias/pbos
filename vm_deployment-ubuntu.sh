...
for i in {1..6}; do ssh -o "StrictHostKeyChecking=no" ubuntu@n$i "sudo apt-get install ntp ntpdate systemd-timesyncd -y && sudo systemctl unmask systemd-timesyncd.service && sudo systemctl enable systemd-timesyncd.service && sudo systemctl start systemd-timesyncd.service
 && sudo systemctl enable ntp && sudo timedatectl set-ntp off && sudo systemctl restart ntp && sudo systemctl status ntp"; done
